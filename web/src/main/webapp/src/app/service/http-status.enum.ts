/**
 * Constants enumerating the HTTP status codes. All status codes defined in RFC1945
 * (HTTP/1.0, RFC2616 (HTTP/1.1), and RFC2518 (WebDAV) are supported.
 *
 *  Adapted from https://github.com/prettymuchbryce/node-http-status/blob/master/index.d.ts
 *
 *  All credits for Bryce Neal (the "node-http-status" repository owner)
...
export enum HttpStatus {
   ...
   * is configured to engage in transparent content negotiation itself, and
   ...
   * from the origin server, but collected from a local or a third party copy. Except
   * this condition, 200 OK response should be preferred instead of this response.
   */
   ...
   * not been modified. So, client can continue to use same cached version of response.
   */
  NOT_MODIFIED = 304,
   ...
   * semantic than the 302 Found HTTP response code, with the exception that
